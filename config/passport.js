...
module.exports = function(passport) {
	...
	*/
	...
	/* 
		OBS: The first argument of deserializeUser corresponds to the key of the 
		user object that was given to the done function (serialize). So your whole 
		object is retrieved with help of that key. That key here is the user id 
		(key can be any key of the user object i.e. name,email etc). In 
		deserializeUser that key is matched with the in memory array / database or
		any data resource.
	...
	*/
	...
	passport.use(
		...
		new LocalStrategy({
			...
			//we will override with matricula
	...
	passport.use(
		...
		function(req, matricula, password, done) { 
			...
			connection.query("SELECT * FROM " + tableSelected + " WHERE matricula = ?",
				...
				//pass to the session the user type to unlock rights and the matricula
